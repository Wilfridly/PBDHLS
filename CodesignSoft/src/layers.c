  ...
  for (i = 0; i < 50; i++) {
    for (j = 0; j < 5; j++)
      for (k = 0; k < 5; k++) {
        ...
        for (m = 0; m < 5; m++)
          for (n = 0; n < 5; n++) {
            somme += Layer2_Weights_CPU[26 * 6 * i + 1 + 6 * (n + 5 * m)] *
                     Layer2_Neurons_CPU[13 * 13 * 0 + 13 * (2 * j + m) +
                                        (2 * k + n)];
            somme += Layer2_Weights_CPU[26 * 6 * i + 1 + 6 * (n + 5 * m) + 1] *
                     Layer2_Neurons_CPU[13 * 13 * 1 + 13 * (2 * j + m) +
                                        (2 * k + n)];
            somme += Layer2_Weights_CPU[26 * 6 * i + 1 + 6 * (n + 5 * m) + 2] *
                     Layer2_Neurons_CPU[13 * 13 * 2 + 13 * (2 * j + m) +
                                        (2 * k + n)];
            somme += Layer2_Weights_CPU[26 * 6 * i + 1 + 6 * (n + 5 * m) + 3] *
                     Layer2_Neurons_CPU[13 * 13 * 3 + 13 * (2 * j + m) +
                                        (2 * k + n)];
            somme += Layer2_Weights_CPU[26 * 6 * i + 1 + 6 * (n + 5 * m) + 4] *
                     Layer2_Neurons_CPU[13 * 13 * 4 + 13 * (2 * j + m) +
                                        (2 * k + n)];
            somme += Layer2_Weights_CPU[26 * 6 * i + 1 + 6 * (n + 5 * m) + 5] *
                     Layer2_Neurons_CPU[13 * 13 * 5 + 13 * (2 * j + m) +
                                        (2 * k + n)];
          }
...
#pragma HLS INTERFACE m_axi port=Layer2_Neurons_CPU offset=slave bundle=gmem
#pragma HLS INTERFACE m_axi port=Layer3_Neurons_CPU offset=slave bundle=gmem

#pragma HLS INTERFACE s_axilite port=Layer2_Neurons_CPU bundle=control
#pragma HLS INTERFACE s_axilite port=Layer3_Neurons_CPU bundle=control
...
// #pragma HLS ARRAY_PARTITION variable=Layer2_Neurons_CPU block factor=78
// #pragma HLS ARRAY_PARTITION variable=Layer2_Weights_CPU block factor=78
// #pragma HLS ARRAY_PARTITION variable=Layer3_Neurons_CPU block factor=78
// #pragma HLS UNROLL factor=4
// #pragma HLS PIPELINE II=1

    float l2_buf[6][13][13];
#pragma HLS ARRAY_PARTITION variable=l2_buf complete dim=1
// #pragma HLS ARRAY_PARTITION variable=l2_buf cyclic factor=2 dim=1
...
#pragma HLS PIPELINE
                l2_buf[c][y][x] =
                    Layer2_Neurons_CPU[c * 13 * 13 + y * 13 + x];
            }
        }
    }
...
// #pragma HLS PIPELINE II=1
						...
						somme += Layer2_Weights_CPU[26*6*i+1+6*(n+5*m)+1] * Layer2_Neurons_CPU[13*13*1+13*(2*j+m)+(2*k+n)]; // line 89
						...
						/* somme += Layer2_Weights_CPU[26*6*i + 1 + 6*(n + 5*m)    ] * l2_buf[0][2*j + m][2*k + n];
						somme += Layer2_Weights_CPU[26*6*i + 1 + 6*(n + 5*m) + 1] * l2_buf[1][2*j + m][2*k + n];
						somme += Layer2_Weights_CPU[26*6*i + 1 + 6*(n + 5*m) + 2] * l2_buf[2][2*j + m][2*k + n];
						somme += Layer2_Weights_CPU[26*6*i + 1 + 6*(n + 5*m) + 3] * l2_buf[3][2*j + m][2*k + n];
						somme += Layer2_Weights_CPU[26*6*i + 1 + 6*(n + 5*m) + 4] * l2_buf[4][2*j + m][2*k + n];
						somme += Layer2_Weights_CPU[26*6*i + 1 + 6*(n + 5*m) + 5] * l2_buf[5][2*j + m][2*k + n]; */
						for (int c = 0; c < 6; c++) {
						#pragma HLS unroll factor=2
						    somme +=
						        Layer2_Weights_CPU[26*6*i + 1 + 6*(n + 5*m) + c] *
						        l2_buf[c][2*j + m][2*k + n];
						}
					}
				Layer3_Neurons_CPU[5*5*i+5*j+k] = (float) SIGMOID(somme);
			}
